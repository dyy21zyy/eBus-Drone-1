...
class EBusDroneEnv:
    ...
    def step(self, action):
        ...
        if selected_action not in feasible:
            action_repaired=True
            cand=[a for a in feasible if a<=selected_action]
            selected_action=max(cand) if cand else 0
        ...
        return self._build_observation(), float(reward), terminated, False, {"event": e, "reward_components": reward_components, "action_repaired": action_repaired, "selected_action_index": int(selected_action), "executed_action_index": int(action), "executed_duration": A_FULL[int(action)]}
